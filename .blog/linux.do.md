...
### 5. 深入底层的节点健康体检
除了关注跑了多少流量，评估节点本身的质量同样是运维的痛点：这个 IP 干不干净？会不会被流媒体平台封杀？网络线路到底好不好？
借由系统内置的 `testing-nodes` 诊断探针，Agent 能够直接通过 SSH 深入服役中的服务器，分别调用业界权威的 IPQuality 和 NetQuality 跑一次全面体检。
不需要去各处找评测脚本，你只要说一句："帮我测一下东京节点的流媒体解锁和 IP 风险得分"，Agent 就会潜入服务器完成探测，并把多维度（比如是否家宽原生、五大数据库的欺诈评分、各大流媒体平台的具体解锁状态）的体检报告清清楚楚地端到你面前。
